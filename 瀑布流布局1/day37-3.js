...
var waterfall = function(wrap,boxes){
	//获取屏幕可以显示的宽度
	var boxWidth = boxes.eq(0).width() + 40;//boxes.eq(0)取到第一个盒子 width取到的是内容的宽度
	...
	for(var i = 0; i<boxes.length;i++){
		if(i < colsNumber){
			everyHeight[i] = boxes.eq(i).height() + 40;// height取到的是内容的高度
		}else{
			//获取最小列的高度
...
var getCheck = function(wrap){
	...
	var lastBoxTop = boxes.eq(boxes.length -1).offset().top;//boxes.eq(boxes.length -1)最后一个盒子   top值
...
var appendBox =function(wrap){
	if(getCheck(wrap)){
		for(i in data){
		var innerString ='<div><img src="day37-img/'+ data[i].src +'"><a href="http://www.imooc.com" target="_blank">'+ data[i].title +'</a></div>'
